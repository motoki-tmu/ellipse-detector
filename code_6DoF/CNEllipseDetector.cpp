...
float V4SPTh = 0.3f;   // 円弧が同一の楕円のものかの判定
...
void CNEllipseDetector::SetParameters(	cv::Size GaussKernelSize,
										double	 GaussSigma,
										double 	 CannyHighTh,
										double   CannyLowTh,
										int		 MinEdgeLength,
...
{
	_GaussKernelSize = GaussKernelSize;
	_GaussSigma = GaussSigma;
	_CannyHighTh = CannyHighTh;
	_CannyLowTh = CannyLowTh;
	_MinEdgeLength = MinEdgeLength;
	_MinRectLength = MinRectLength;
	_StringNum = StringNum;
	_PositionTh = PositionTh;
	_V4SPTh = V4SPTh;
	_MaxCenterDistance = MaxCenterDistance;
	_MaxCenterDistance2 = MaxCenterDistance * MaxCenterDistance;
	_ContourTh = ContourTh;
...
void CNEllipseDetector::DetectEdges13(cv::Mat1b& DP, VVP& points_1, VVP& points_3)
{
	...
	Labeling(DP, contours, _MinEdgeLength);  // contoursに独立したエッジを構成するピクセルの座標がラベル別に格納される
...
void CNEllipseDetector::DetectEdges24(cv::Mat1b& DN, VVP& points_2, VVP& points_4 )
{
	...
	Labeling(DN, contours, _MinEdgeLength);
...
void CNEllipseDetector::GetFastCenter(VP& e1, VP& e2, EllipseData& data)
{
	...
	{// e1の始点とe2の中点
		...
		unsigned minPoints = (_StringNum < hsize_2) ? _StringNum : hsize_2; // 最大_uNs個の点を探索

		std::vector<uint> indexes(minPoints);
		if (_StringNum < hsize_2) // サンプリング処理
		{
			unsigned iSzBin = hsize_2 / unsigned(_StringNum);
			unsigned iIdx = hsize_2 + (iSzBin / 2);

			for (unsigned i = 0; i < _StringNum; ++i)
			{
				indexes[i] = iIdx;
				iIdx += iSzBin;
			}
	...
	{// e2の始点とe1の中点
		...
		uint minPoints = (_StringNum < hsize_1) ? _StringNum : hsize_1;

		std::vector<uint> indexes(minPoints);
		if (_StringNum < hsize_1)
		{
			unsigned iSzBin = hsize_1 / unsigned(_StringNum);
...
void CNEllipseDetector::FindEllipses(cv::Point2f& center, VP& edge_i, VP& edge_j, VP& edge_k, EllipseData& data_ij, EllipseData& data_ik, std::vector<Ellipse>& ellipses)
{
	...
	for (ushort l = 0; l < sz_ei; ++l)
	{
		...
		if (abs(h - 1.f) < _ContourTh)   // ピクセルが輪郭上にあれば、h=1になるはず（ContourTh = 0.1f）
		{
...
void CNEllipseDetector::Triplets124(VVP& pi, VVP& pj, VVP& pk, std::unordered_map<uint, EllipseData>& data, std::vector<Ellipse>& ellipses)
{
	...
	for (ushort i = 0; i < sz_i; ++i)
	{
		...
		for (ushort j = 0; j < sz_j; ++j)
		{
			...
			if (pjl.x > pif.x + _PositionTh) // _PositionTh = 1.0 はピクセルの許容誤差
			{
				continue;
			}
#endif
			...
			if(myselect1 && fabs(value4SixPoints(T124) - 1) > V4SPTh) // V4SPTh = 0.3　→　不変量が1.3以上なら違う円弧とみなしてcontinue
			{
				continue;
			}
#endif
			...
			for (ushort k = 0; k < sz_k; ++k)
			{
				...
				if(myselect2 && fabs(value4SixPoints(pif, pim, pil, pkf, pkm, pkl) - 1) > V4SPTh)
				{
					continue;
				}

				if(myselect3 && fabs(value4SixPoints(pjf, pjm, pjl, pkf, pkm, pkl) - 1) > V4SPTh)
				{
					continue;
				}
#endif
...
#ifndef DISCARD_CONSTRAINT_CENTER // 2つの推定結果の差分が閾値内かチェック（_MaxCenterDistance2 = 25）
				if (ed2(data_ij.Cab, data_ik.Cab) > _MaxCenterDistance2)
				{
					continue;
				}
#endif
...
#ifndef DISCARD_CONSTRAINT_CNC
			if(myselect1 && fabs(value4SixPoints(T231) - 1) > V4SPTh)
			{
				continue;
			}
#endif
...
#ifndef DISCARD_CONSTRAINT_CNC
			if(myselect1 && fabs(value4SixPoints(T342) - 1) > V4SPTh)
			{
				continue;
			}
#endif
...
#ifndef DISCARD_CONSTRAINT_CNC
			if(myselect1 && fabs(value4SixPoints(T413) - 1) > V4SPTh)
			{
				continue;
			}
#endif
...
#ifndef DISCARD_CONSTRAINT_CNC // value4SixPointsは円弧から得られる不変量で、この値がほとんど変わらなければ同一円弧とみなす。詳細はtools.cpp参照
			if(myselect1 && fabs(value4SixPoints(D12) - 1) > V4SPTh) // V4SPTh = 0.3　→　不変量が1.3以上なら違う円弧とみなしてcontinue
			{
				continue;
			}
#endif
...
#ifndef DISCARD_CONSTRAINT_CNC
			if(myselect1 && fabs(value4SixPoints(D23) - 1) > V4SPTh)
			{
				continue;
			}
#endif
...
#ifndef DISCARD_CONSTRAINT_CNC
			if(myselect1 && fabs(value4SixPoints(D34) - 1) > V4SPTh)
			{
				continue;
			}
#endif
...
#ifndef DISCARD_CONSTRAINT_CNC
			if(myselect1 && fabs(value4SixPoints(D41) - 1) > V4SPTh)
			{
				continue;
			}
#endif
...
#ifndef DISCARD_CONSTRAINT_CNC
			if(myselect1 && fabs(value4SixPoints(D13) - 1) > V4SPTh)
			{
				continue;
			}
#endif
...
#ifndef DISCARD_CONSTRAINT_CNC
			if(myselect1 && fabs(value4SixPoints(D24) - 1) > V4SPTh)
			{
				continue;
			}
#endif
